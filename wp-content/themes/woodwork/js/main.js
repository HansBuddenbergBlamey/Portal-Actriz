(function (window, undefined) {
    ...
    WW.ProjectBar = function ($element) {
        ...
        this.handleClick = function () {
            ...
            self.$element.stop(true, true).css({
                left: 0
            });
    ...
    WW.ScrollingCarousel = {
        ...
        init: function ($element) {
            ...
            WW.ScrollingCarousel.$element.animate({
                scrollLeft: 0
            }, 10);
            ...
            WW.ScrollingCarousel.$element.find('.bar').each(function () {
                ...
                bar.$element.css({
                    'z-index': WW.ScrollingCarousel.barCount - i
                });
        ...
        addListeners: function () {
            ...
            if (Modernizr.touch) {
                WW.ScrollingCarousel.$element.css({
                    'overflow-x': 'scroll',
        ...
        closeProjectView: function (callback) {
            ...
            WW.ScrollingCarousel.$elementInner.animate({
                width: WW.ScrollingCarousel.carouselWidth
            }, 1000, 'easeInOutQuad');
        ...
        hideCarousel: function () {
            ...
            WW.ScrollingCarousel.$element.stop().animate({
                scrollLeft: 0
            }, 500, 'linear').animate({
                'margin-left': w
            }, 500, 'easeInOutQuad');
    ...
    WW.ProjectOverlay = {
        ...
        addListeners: function () {
            ...
            WW.ProjectOverlay.$element.find('.video').live('click', function (e) {
                e.preventDefault();
                $(this).fadeOut();

                var videoURL = $(this).find('.play-button').data('videourl');
                ...
                WW.ProjectOverlay.$element.find('#video-player').html($player);

                WW.ProjectOverlay.stopVideoSlideshow();
            });

            WW.ProjectOverlay.$element.find('#video-controls a').live('click', function (e) {
                e.preventDefault();
                WW.ProjectOverlay.replaceVideo($(this));
                WW.ProjectOverlay.stopVideoSlideshow();
            });

            if (!Modernizr.touch) {
                if (WW.ProjectOverlay.$element.find('#video-controls a').length > 0) {
                    WW.ProjectOverlay.startVideoSlideshow();
                }
            }
            ...
            new WW.MagicButton(WW.ProjectOverlay.$element.find('#video-controls a'), 15);
        ...
        resizeVideoPlayer: function () {
            ...
            WW.ProjectOverlay.$element.find('#videos, #videos .video img').css({
                'width': w,
                'height': Math.floor((w / 16) * 9)
            });

            WW.ProjectOverlay.$element.find('#videos').css({
                'margin-top': Math.floor((WW.ProjectOverlay.$element.width() - w) * .2)
            });

            WW.ProjectOverlay.$element.find('.video .play-button').css({
                'left': w / 2
            });
        },
        ...
        setContent: function (content) {
            ...
            if (WW.ScrollingCarousel.barActive != null) {
                ...
                setTimeout(function () {
                    ...
                    WW.ProjectOverlay.checkMobileDevicePlayer();
                }, 750);
            } else {
                // wait 0
        ...
        checkMobileDevicePlayer: function () {
            if (Modernizr.touch) {
                var $activeVideo = WW.ProjectOverlay.$element.find('#video-holder .video.active');
                var videoURL = $activeVideo.find('.play-button').data('videourl');
                $activeVideo.hide();
        ...
        replaceVideo: function ($this) {

            if ($this.hasClass('active')) {
                return false;
            }

            var $controls = WW.ProjectOverlay.$element.find('#video-controls a');
            var $videos = WW.ProjectOverlay.$element.find('.video');

            WW.ProjectOverlay.$element.find('#video-player').html('');

            $controls.removeClass('active');
            $this.addClass('active');

            $videos.eq($this.index()).css('width', 0).addClass('top active').animate({
                width: '100%'
            }, 500, 'easeOutQuad', function () {
                $(this).removeClass('top');
                $videos.not($(this)).css('display', '').removeClass('active');
                WW.ProjectOverlay.checkMobileDevicePlayer();
            });
        },

        startVideoSlideshow: function () {
            WW.ProjectOverlay.stopVideoSlideshow();

            WW.ProjectOverlay.videoSlideshowInterval = setInterval(function () {
                var $next = WW.ProjectOverlay.$element.find('#video-controls a.active').next('a');
                if ($next.length == 0) {
                    $next = WW.ProjectOverlay.$element.find('#video-controls a').first();
                }
                WW.ProjectOverlay.replaceVideo($next);
            }, 5000);
        },

        stopVideoSlideshow: function () {
            clearInterval(WW.ProjectOverlay.videoSlideshowInterval);
        }
    };
    ...
    WW.StandardPage = {
        ...
        setContent: function (content, delay) {
            setTimeout(function () {
                ...
                WW.StandardPage.initContactMap();
            }, delay);
        },
        ...
        initContactMap: function () {
            if ($('#contact-map').length == 0) return;
            ...
            WW.StandardPage.map = new google.maps.Map(document.getElementById("contact-map"), {
                center: new google.maps.LatLng(52.371652, 4.879139),
                zoom: 14,
            ...
            var marker = new google.maps.Marker({
                position: new google.maps.LatLng(52.371652, 4.879139),
                map: WW.StandardPage.map,
                icon: {
                    url: TEMPLATE_URL + '/img/contact-marker.png',
                    size: new google.maps.Size(61, 61),
                    origin: new google.maps.Point(0, 0),
                    anchor: new google.maps.Point(30, 61)
                },
                shadow: {
                    url: TEMPLATE_URL + '/img/contact-marker-shadow.png',
                    size: new google.maps.Size(71, 47),
                    origin: new google.maps.Point(0, 0),
            ...
            google.maps.event.addListener(marker, 'click', function () {
                window.open('http://maps.google.nl/maps?daddr=Woodwork+Amsterdam,+Laurierstraat+103-105,+1016+PL+Amsterdam&hl=en&t=m&z=16');
            });
        ...
        addListeners: function () {
            ...
            $('#back-to-home').live('mouseover', function () {
                WW.ScrollingCarousel.$element.stop().animate({
                    'margin-left': WW.ScrollingCarousel.$element.width() - WW.ScrollingCarousel.BAR_HIDDEN_WIDTH - 10
                });
            }).live('mouseout', function () {
                if (WW.ScrollingCarousel.hidden) {
                    WW.ScrollingCarousel.$element.stop().animate({
                        'margin-left': WW.ScrollingCarousel.$element.width() - WW.ScrollingCarousel.BAR_HIDDEN_WIDTH
                    });
                }
            });
        },
        ...
        show: function (delay) {
            ...
            $el.css({
                left: WW.StandardPage.calculateAnimationStartPosition()
            }).show();

            setTimeout(function () {
                $el.animate({
                    left: WW.StandardPage.startLeftX
                }, 500, 'easeInOutQuad', function () {
        ...
        switchPage: function () {
            ...
            $el.animate({
                left: WW.StandardPage.startLeftX + 100,
                opacity: 0
            }, 200, function () {
                $el.css({
                    left: WW.StandardPage.startLeftX - 100
        ...
        resize: function () {
            ...
            //else {
            $el.css({
                'padding-top': WW.StandardPage.MIN_TOP_DISTANCE
            });
            //}
    ...
    WW.NavigationBar = {
        ...
        highlightMenuItem: function ($item) {
            $('#nav-pointer').stop().animate({
                top: $item.position().top + 16
            }, 500, 'easeOutBack');
            $('#nav-pointer').find('#pointer-left').stop().animate({
                left: $item.position().left - 5
            }, 500);
            $('#nav-pointer').find('#pointer-right').stop().animate({
        ...
        resize: function () {
            ...
            WW.NavigationBar.$element.css({
                'padding-top': Math.max(p, WW.NavigationBar.MIN_TOP_DISTANCE)
            });
        }
    };
    ...
    WW.NavigationManager = {
        ...
        addListeners: function () {
            ...
            $('a:not(#play-button,.play-button,.video-switch)').live('click', function (event) {
                var
                    $this = $(this),
                    url = $this.attr('href'),
                ...
                title = (title != '') ? title + ' | Ximena Marchant' : 'Ximena Marchant';

                // Continue as normal for cmd clicks etc
                if (event.which == 2 || event.metaKey || typeof $this.attr('target') != 'undefined' || url.indexOf('mailto:') > -1) {
